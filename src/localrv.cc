            ...
            switch (type) {
				case WAVE_SUB_SCOPE:
					if(strategy == DOMAIN_LOCAL){
						wave_handle_subscription(_remotehost, ID, prefixID, strategy) ;
					}
					else{
						//only accept DOMAIN_LOCAL
						result = false ;
					}
					break ;
                case PUBLISH_SCOPE:
...
void LocalRV::wave_handle_subscription(RemoteHost *_subscriber, String &ID, String &prefixID, unsigned char strategy)
{
	unsigned int ret;
	Scope *sc = NULL;
	String fullID = prefixID+ID ;
	...
	if (sc == scopeIndex.default_value()) {
		...
		if (sc->strategy == strategy) {
			...
			unsigned int noofchunk = _subscopes.size() ;
			...
			wave_rendezvous(fullID, publishers, _subscriber, noofchunk) ;
			ret = SUCCESS;
		} else {
			click_chatter("LocalRV: strategies don't match....aborting subscription");
			ret = STRATEGY_MISMATCH;
...
void LocalRV::wave_rendezvous(String &fullID, RemoteHostSet &publishers, RemoteHost* subscriber, unsigned int noofchunk)
{
	/********FOR THE API*********/
	unsigned char typeForAPI = PUBLISH_DATA;
	unsigned char IDLenForAPI = 2 * PURSUIT_ID_LEN / PURSUIT_ID_LEN;
	unsigned char strategyAPI = IMPLICIT_RENDEZVOUS;
	/****************************/
	unsigned char type = WAVE_NOTIFY_SUB ;
	unsigned char noofpub = publishers.size() ;
	WritablePacket *p;
	unsigned int packet_len ;
	unsigned char IDlen = (fullID.length()) / PURSUIT_ID_LEN ;
	/*allocate the packet*/
	packet_len = /*For the blackadder API*/ sizeof (typeForAPI) + sizeof (IDLenForAPI) + gc->nodeTMScope.length() + sizeof (strategyAPI) + FID_LEN/*END OF API*/\
		/*PAYLOAD*/ + sizeof(type) + sizeof(noofpub) + noofpub*NODEID_LEN + NODEID_LEN + sizeof(noofchunk) + sizeof(IDlen) + fullID.length() ;
	p = Packet::make(50, NULL, packet_len, 0);
	/*For the API*/
	memcpy(p->data(), &typeForAPI, sizeof (typeForAPI));
	memcpy(p->data() + sizeof (typeForAPI), &IDLenForAPI, sizeof (IDLenForAPI));
	...
	memcpy(p->data() + sizeof (typeForAPI) + sizeof (IDLenForAPI) + gc->nodeTMScope.length() + sizeof (strategyAPI), gc->TMFID._data, FID_LEN);
	/*Put the payload*/
	unsigned int API_len = sizeof (typeForAPI) + sizeof (IDLenForAPI) + gc->nodeTMScope.length() + sizeof (strategyAPI) + FID_LEN ;
	memcpy(p->data() + API_len, &type, sizeof(type)) ;
	memcpy(p->data() + API_len + sizeof(type), &noofpub, sizeof(noofpub)) ;
	int pub_index = 0 ;
	for (RemoteHostSetIter iter = publishers.begin(); iter != publishers.end(); iter++) {
		memcpy(p->data() + API_len + sizeof(type) + sizeof(noofpub) + pub_index,\
			   (*iter)._rhpointer->remoteHostID.c_str(), (*iter)._rhpointer->remoteHostID.length()) ;
		pub_index += (*iter)._rhpointer->remoteHostID.length();
	}
	memcpy(p->data() + API_len + sizeof(type) + sizeof(noofpub) + pub_index, subscriber->remoteHostID.c_str(), subscriber->remoteHostID.length()) ;
	memcpy(p->data() + API_len + sizeof(type) + sizeof(noofpub) + pub_index + subscriber->remoteHostID.length(), &noofchunk, sizeof(noofchunk)) ;
	memcpy(p->data() + API_len + sizeof(type) + sizeof(noofpub) + pub_index + subscriber->remoteHostID.length() + sizeof(noofchunk),\
		   &IDlen, sizeof(IDlen)) ;
	memcpy(p->data() + API_len + sizeof(type) + sizeof(noofpub) + pub_index + subscriber->remoteHostID.length() + sizeof(noofchunk) + sizeof(IDlen),\
		   fullID.c_str(), fullID.length()) ;
