...
void LocalProxy::push(int in_port, Packet * p) {
	...
	if (in_port == 4){
		...
		memcpy(&chunkno, p->data(), sizeof(chunkno)) ;
		type = *(p->data()+sizeof(chunkno)) ;
		if( type == WAVE_PUBLISH_DATA )
		{
			...
			idlen = *(p->data()+sizeof(chunkno)+1) ;
			fullID = String( (const char*) (p->data()+sizeof(chunkno)+2), idlen*PURSUIT_ID_LEN) ;
			memcpy(&totalinfonum, p->data()+sizeof(chunkno)+2+idlen*PURSUIT_ID_LEN, sizeof(totalinfonum)) ;
			...
			memcpy(datapacket->data()+2, fullID.c_str(), fullID.length()) ;
			as = activeSubscriptionIndex.get(fileID) ;
			if(as == activeSubscriptionIndex.default_value())
			{
				click_chatter("can not find subscriber") ;
				datapacket->kill() ;
				return ;
			}else{
				int counter = 1 ;
				WritablePacket* tempp ;
				for(LocalHostSetIter iter = as->subscribers.begin() ;iter != as->subscribers.end() ; iter++)
				{
					if(counter == as->subscribers.size())
						tempp = datapacket->uniqueify() ;
					else
				...
				as->infonum_for_each_chunk++ ;
				if( as->infonum_for_each_chunk == totalinfonum )
				{
					as->infonum_for_each_chunk = 0 ;
					if( chunkno == as->noofchunk )
					{//notify sub file receiving completes
						counter = 1 ;
						as->noofchunk = 0 ;
						unsigned char ntype ;
						ntype = WAVE_DOWNLOAD_FINISHED ;
						WritablePacket* np ;
						np = Packet::make(20, NULL, 1+sizeof(idlen)+idlen*PURSUIT_ID_LEN,20) ;
						memcpy(np->data(), &ntype, 1) ;
						memcpy(np->data()+1, &idlen, 1) ;
						memcpy(np->data()+2, fileID.c_str(), fileID.length()) ;
		...
		switch(type){
			case WAVE_RETRIEVE_CONTENT:
			{
				...
				unsigned int totalnumchunk ;
				memcpy(fid2sub._data, p->data()+sizeof(type), FID_LEN) ;
				IDlen = *(p->data()+sizeof(type)+FID_LEN) ;
				fileID = String((const char*)(p->data()+sizeof(type)+FID_LEN+sizeof(IDlen)), IDlen*PURSUIT_ID_LEN) ;
				chunkid = String((const char*)(p->data()+sizeof(type)+FID_LEN+sizeof(IDlen)+IDlen*PURSUIT_ID_LEN), PURSUIT_ID_LEN) ;
				memcpy(&chunkno, p->data()+sizeof(type)+FID_LEN+sizeof(IDlen)+(IDlen+1)*PURSUIT_ID_LEN, sizeof(chunkno)) ;
				memcpy(&totalnumchunk, p->data()+sizeof(type)+FID_LEN+sizeof(IDlen)+(IDlen+1)*PURSUIT_ID_LEN+sizeof(chunkno), sizeof(totalnumchunk)) ;
				String filechunkid = fileID+chunkid ;
				ActivePublication *ap ;
				ap = activePublicationIndex.get(filechunkid);

				if (ap != activePublicationIndex.default_value()) {
					/*this item exists*/
					Vector<String> infoID ;
					infoID = ap->infoID ;
					ap->FID_to_subscribers = fid2sub;
					for( Vector<String>::iterator iter = infoID.begin() ; iter != infoID.end() ; iter++ )
					{
					    String tempfullID = filechunkid+(*iter) ;
						...
						for (PublisherHashMapIter publishers_it = ap->publishers.begin(); publishers_it != ap->publishers.end(); publishers_it++) {
							//find the publisher, and notify it
							(*publishers_it).second = START_PUBLISH;
							WAVE_sendNotificationLocally((unsigned char)WAVE_START_PUBLISH, (*publishers_it).first, tempfullID, fid2sub, chunkno, totalnumchunk);
							break ;
						}
					}
				}
...
bool LocalProxy::storeActivePublication(LocalHost *_publisher, String &fullID, unsigned char strategy, BABitvector &RVFID, bool isScope) {
	...
	String filechunkid ;
	if( fullID.length() > 2*PURSUIT_ID_LEN )
		 filechunkid = fullID.substring(0, fullID.length()-PURSUIT_ID_LEN) ;
    if ((strategy == NODE_LOCAL) || (strategy == DOMAIN_LOCAL)) {
        ...
        if (ap == activePublicationIndex.default_value()) {
            ...
            fatherscope = activePublicationIndex.get(filechunkid) ;
			if( fullID.length() > 2*PURSUIT_ID_LEN )
				fatherscope->infoID.push_back(fullID.substring(fullID.length()-PURSUIT_ID_LEN, PURSUIT_ID_LEN)) ;
			return true;
        } else {
...
void LocalProxy::handleRVNotification(Packet *p) {
    ...
    BABitvector FID;
    type = *(p->data());
	if(type == WAVE_NOTIFY_SUB)
	{
		BABitvector fid2pub(FID_LEN*8) ;
		unsigned int noofchunk = 0 ;
		unsigned char idlen = 0 ;
		String fileID ;
		...
		as = activeSubscriptionIndex.get(fileID);
		if (as != activeSubscriptionIndex.default_value()) {
			as->noofchunk = noofchunk ;
			as->fid2pub = fid2pub ;
			unsigned char sub_type = WAVE_RETRIEVE_CONTENT ;
			unsigned int retrieveID = 1 ;//begin with the first chunk
			String chunkid = gc->calculatCHUNKID(retrieveID) ;
			WritablePacket *sub_mes ;
			unsigned int packet_len = FID_LEN+sizeof(sub_type)+FID_LEN+sizeof(idlen)+fileID.length()+chunkid.length()+sizeof(retrieveID)+sizeof(noofchunk) ;
			sub_mes = Packet::make(50, NULL, packet_len, 0);
			memcpy(sub_mes->data(), fid2pub._data, FID_LEN) ;
			memcpy(sub_mes->data()+FID_LEN, &sub_type, sizeof(sub_type)) ;
			memcpy(sub_mes->data()+FID_LEN+sizeof(sub_type), gc->iLID._data, FID_LEN) ;
			memcpy(sub_mes->data()+FID_LEN+sizeof(sub_type)+FID_LEN, &idlen, sizeof(idlen)) ;
			memcpy(sub_mes->data()+FID_LEN+sizeof(sub_type)+FID_LEN+sizeof(idlen), fileID.c_str(), fileID.length()) ;
			memcpy(sub_mes->data()+FID_LEN+sizeof(sub_type)+FID_LEN+sizeof(idlen)+fileID.length(), chunkid.c_str(), chunkid.length()) ;
			memcpy(sub_mes->data()+FID_LEN+sizeof(sub_type)+FID_LEN+sizeof(idlen)+fileID.length()+chunkid.length(), &retrieveID, sizeof(retrieveID)) ;
			memcpy(sub_mes->data()+FID_LEN+sizeof(sub_type)+FID_LEN+sizeof(idlen)+fileID.length()+chunkid.length()+sizeof(retrieveID), &noofchunk, sizeof(noofchunk)) ;
			output(3).push(sub_mes) ;
		}
		p->kill() ;
		return ;
...
void LocalProxy::WAVE_sendNotificationLocally(unsigned char type, LocalHost *_localhost, String& ID,\
                                              BABitvector& fid2sub, unsigned int chunkno, unsigned int totalchunknum) {
	WritablePacket *p;
	unsigned char IDLength;
	unsigned int packetlen = 0 ;
	packetlen = sizeof (unsigned char)+sizeof(unsigned char)+ID.length()+FID_LEN+sizeof(chunkno)+sizeof(totalchunknum) ;
	p = Packet::make(30, NULL, packetlen, 0);
	IDLength = ID.length() / PURSUIT_ID_LEN;
	memcpy(p->data(), &type, sizeof (char));
	memcpy(p->data() + sizeof (unsigned char), &IDLength, sizeof (unsigned char));
	memcpy(p->data() + sizeof (unsigned char) + sizeof (unsigned char), ID.c_str(), IDLength * PURSUIT_ID_LEN);
	memcpy(p->data() + sizeof (unsigned char) + sizeof (unsigned char)+IDLength * PURSUIT_ID_LEN, fid2sub._data, FID_LEN) ;
	memcpy(p->data() + sizeof (unsigned char) + sizeof (unsigned char)+IDLength * PURSUIT_ID_LEN+FID_LEN, &chunkno, sizeof(chunkno)) ;
	memcpy(p->data() + sizeof (unsigned char) + sizeof (unsigned char)+IDLength * PURSUIT_ID_LEN+FID_LEN+sizeof(chunkno), &totalchunknum, sizeof(totalchunknum)) ;
	if (_localhost->type == CLICK_ELEMENT) {
		output(_localhost->id).push(p);
	} else {
		/*set the annotation for the to_netlink element*/
