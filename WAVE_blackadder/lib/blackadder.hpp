...
public:
	...
	/**@brief this method is only for WAVE implementation*/
	void wave_subscribe_scope(const string&id, const string&prefix_id, unsigned char strategy, void *str_opt, unsigned int str_opt_len);
	/**@brief this method is only for WAVE implementation*/
    void wave_publish_data(string& id, void *fidinfo, unsigned int fidlen,\
		void* data, unsigned int data_len, unsigned int chunkno, unsigned int totalchunknum, unsigned int totalinfonum) ;
