...
void Blackadder::wave_subscribe_scope(const std::string &id, const std::string &prefix_id, unsigned char strategy, void *str_opt, unsigned int str_opt_len)
{
	int ret;
	if (id.length() % PURSUIT_ID_LEN != 0) {
		cout << "Blackadder Library: Could not send SUBSCRIBE_SCOPE request - wrong ID size" << endl;
		...
		ret = create_and_send_buffers(WAVE_SUB_SCOPE, id, prefix_id, strategy, str_opt, str_opt_len);
		if (ret < 0) {
			perror("Blackadder Library: Could not send SUBSCRIBE_SCOPE request");
		}
	}
}
void Blackadder::wave_publish_data(std::string &id, void *fidinfo, unsigned int fidlen, void *data, unsigned int data_len,\
								   unsigned int chunkno, unsigned int totalchunknum, unsigned int totalinfonum)
{
	int ret;
	struct msghdr msg;
	struct iovec *iov;
	if (id.length() % PURSUIT_ID_LEN != 0) {
		...
		unsigned char type = WAVE_PUBLISH_DATA;
		unsigned char id_len = id.length() / PURSUIT_ID_LEN;
		struct nlmsghdr *nlh = (struct nlmsghdr *) malloc(sizeof (struct nlmsghdr));
		unsigned char cachesetflag = 0 ;
		/* Fill the netlink message header */
		nlh->nlmsg_len = sizeof (struct nlmsghdr)+1/*type*/+FID_LEN+1/*for id length*/+\
						 id.length()+sizeof(cachesetflag)+sizeof(chunkno)+sizeof(totalchunknum)+data_len;
		nlh->nlmsg_pid = getpid();
		nlh->nlmsg_flags = 1;
		nlh->nlmsg_type = 0;
		iov = (struct iovec *) calloc(sizeof (struct iovec), 10);
		iov[0].iov_base = nlh;
		iov[0].iov_len = sizeof (struct nlmsghdr);
		iov[1].iov_base = &type;
		iov[1].iov_len = sizeof (type);
		iov[2].iov_base = (void*) fidinfo;
		iov[2].iov_len = FID_LEN;
		iov[3].iov_base = &id_len ;
		iov[3].iov_len = sizeof(id_len) ;
		iov[4].iov_base = (void *) id.c_str();
		iov[4].iov_len = id.length();
		iov[5].iov_base = &cachesetflag;
		iov[5].iov_len = sizeof(cachesetflag);
		iov[6].iov_base = &chunkno ;
		iov[6].iov_len = sizeof(chunkno);
		iov[7].iov_base = &totalchunknum ;
		iov[7].iov_len = sizeof(totalchunknum);
		iov[8].iov_base = &totalinfonum;
		iov[8].iov_len = sizeof(totalinfonum);
		iov[9].iov_base = (void *) data;
		iov[9].iov_len = data_len;
		memset(&msg, 0, sizeof (msg));
		msg.msg_name = (void *) &d_nladdr;
		msg.msg_namelen = sizeof (d_nladdr);
		msg.msg_iov = iov;
...
again:
    ...
    if (total_buf_size > 0) {
		...
		if(ev.type == WAVE_START_PUBLISH)
		{
			unsigned int tempchunkno ;
			unsigned int temptotalnum ;
			ev.fid2sub.resize(FID_LEN*8) ;
			memcpy(ev.fid2sub._data, ((char *)ev.buffer+sizeof(struct nlmsghdr)+sizeof(unsigned char)+sizeof(unsigned char)+((int)id_len)*PURSUIT_ID_LEN), FID_LEN) ;
			memcpy(&tempchunkno, (char *)(ev.buffer+sizeof(struct nlmsghdr)+sizeof(unsigned char)+sizeof(unsigned char)+((int)id_len)*PURSUIT_ID_LEN)+FID_LEN, sizeof(tempchunkno)) ;
			memcpy(&temptotalnum, (char *)(ev.buffer+sizeof(struct nlmsghdr)+sizeof(unsigned char)+sizeof(unsigned char)+((int)id_len)*PURSUIT_ID_LEN)+FID_LEN+sizeof(tempchunkno), sizeof(temptotalnum)) ;
			ev.chunkno = tempchunkno ;
			ev.totalchunknum = temptotalnum ;
			ev.data = NULL ;
			ev.data_len = 0 ;
		} else if (ev.type == PUBLISHED_DATA || ev.type == WAVE_PUBLISH_DATA) {
