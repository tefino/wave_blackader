...
String GlobalConf::calculatCHUNKID(unsigned int chunkNO)
{
	char char_chunkid[PURSUIT_ID_LEN] ;
	for(int i = 0 ; i < 4 ; i++)
	{
		char_chunkid[i] = 0 ;
	}
	...
	{
		unsigned char tuc = 0 ;
		tuc = (chunkNO >> 8*(3-i))&0x000000FF ;
		char_chunkid[i+4] = tuc ;
	}
	return String(char_chunkid, PURSUIT_ID_LEN) ;
}
