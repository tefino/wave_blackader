...
void Domain::writeClickFiles(bool montoolstub) {
    ...
    for (int i = 0; i < network_nodes.size(); i++) {
        ...
        if (montoolstub && (nn->running_mode.compare("user") == 0)) {
            click_conf << "require(WAVE_blackadder); \n\nControlSocket(\"TCP\",55555);\n\n " << endl << endl;
        } else {
            click_conf << "require(WAVE_blackadder);" << endl << endl;
        }
        ...
        click_conf << "fw::Forwarder(globalconf," << cache_size << "," << nn->connections.size() << "," << endl;
        for (int j = 0; j < nn->connections.size(); j++) {
            NetworkConnection *nc = nn->connections[j];
            int offset;
            if (overlay_mode.compare("mac") == 0) {
        ...
        if (overlay_mode.compare("mac") == 0) {
            ...
            if (nn->running_mode.compare("user") == 0) {
                click_conf << "classifier::Classifier(12/080a, 12/080b, 12/080c);" << endl;
            } else {
...
void Domain::startClick() {
    ...
    for (int i = 0; i < network_nodes.size(); i++) {
        ...
        if (nn->running_mode.compare("user") == 0) {
            if (sudo) {
                command = "ssh " + user + "@" + nn->testbed_ip + " \"sudo " + click_home + "bin/click " + write_conf + nn->label + ".conf > /tmp/wave_blackadder 2>&1 &\"";
            } else {
                command = "ssh " + user + "@" + nn->testbed_ip + " \"" + click_home + "bin/click " + write_conf + nn->label + ".conf > /tmp/wave_blackadder 2>&1 &\"";
            }
            cout << command << endl;
            ssh_command = popen(command.c_str(), "r");
            if (ssh_command == NULL) {
            ...
            if (sudo) {
                command = "ssh " + user + "@" + nn->testbed_ip + " \"sudo " + click_home + "sbin/click-install " + write_conf + nn->label + ".conf > /tmp/wave_blackadder 2>&1 &\"";
            } else {
                command = "ssh " + user + "@" + nn->testbed_ip + " \"" + click_home + "sbin/click-install " + write_conf + nn->label + ".conf > /tmp/wave_blackadder 2>&1 &\"";
            }
            cout << command << endl;
            ssh_command = popen(command.c_str(), "r");
            if (ssh_command == NULL) {
...
string Domain::writeConfigFile(string filename) {
    ...
    for (int i = 0; i < network_nodes.size(); i++) {
        ...
        configfile << "CACHE_SIZE = " << cache_size << ";\n";
